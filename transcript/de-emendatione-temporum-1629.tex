...
\usepackage{booktabs}
%
%% Extension to booktabs: variation on \bottomrule, with room for a footnote
%% (The command \footnoterule already exists, for regular footnotes,
%%  and the command \footrule clashes with the fancyheader package.)
\makeatletter
\newcommand{\tfr@nostar}{\bottomrule}
\newcommand{\tfr@star}{\midrule}
\newcommand{\tabfootrule}{\@ifstar{}{\tfr@nostar}}
\makeatother

%%% Multirow to allow spanning of cells in tables over more than one row
%% [longtable] option fixes a bug in longtable involving \\* and \cline
\usepackage[longtable]{multirow}

%%% Control the way page headers look
%% "fancyheader" package is obsolete
%\usepackage{fancyhdr}
%% NOTE: This package introduces a \footrule command, which clashed with
%% our private \footrule command. So our \footrule was renamed to \tabfootrule
...
\newenvironment{tabnums}{\addfontfeature{RawFeature=
{Number Spacing=Proportional Numbers}}}{}
%% Sep 2022: Changed from "Numbers={Proportional}" to
%% RawFeature={Number Spacing=Proportional Numbers}}
%% due to a bug in TexLive 2020 or possibly poor support in the Apple version
%% of Hoefler Text.
%% We still need to set Proportional to get Monospaced.
%% The raw names come from the developer.apple.com/fonts
%% TrueType Reference Manual - Font Feature Registry
...
%% Register the PDF page number and the source page number
%% Do not print the page number and line number.
%% To be used when a new page has begun but it is impractical to
%% use \plnr{}{}{}, e.g. when there is a section header or a table at the top

...
\verb+nnn+ is the three-digit page number (with leading zeroes as reguired)
where the table appears in the original.
...
If you are satisfied with the result you can copy and past the inclusion
...
to float the tables (except for long tables).
...
small tables (less than 1/3 of the hight of the page).
...
This should happen to tables which are more than 2/3 of the hight of the page.
\end{description}
Tables are floated at paragraph breaks, but the body text of the original
hardly has any paragraph breaks.
To guide the placement of the floating tables try to insert a paragraph break
(in the form of a blank line) near the location in the text where you want
the table to appear.
...
Each column header should be in a \verb+\multicolumn{1}{c}{column header}+
to centre it.
Column headers might become so wide that the table becomes ungainly, or simply
too wide for the page.
You can then split them over two or three lines (Scaliger often does this).
In that case make two (or three) lines of headers with the first line containing
the first part of each header and the second line the second parts, etc.
Headers with fewer lines should be alligned to the bottom, not the top.
For instance: if there is one header with two lines and one header with one line,
the one-line header should go into the second, lowest line of headers.
...
Use \verb+\begin{rotate}{75}...\end{rotate}+ (75 is the amount of rotation,
in degrees) for all headers you want to rotate except the rightmost.
For the rightmost header use \verb+\begin{turn}{75}...\hspace{3em}\end{turn}+.

...
\item Some obvious abbreviations should be expanded
 (see also Diacriticals above).
	...
	\item "Iulianae" is sometimes written with an 'e' with a cedille underneath
	 it instead of the 'ae'. E.G. Prolegomena page~X.
	 Same for "Iudaei".
	\end{itemize}

\item On the other hand, most abbreviated words should be transcribed
unchanged.
Though it is tempting to expand these to full words, any time we do this
would mean we second-guess Scaliger. 
A frequently occurring example is "Kal.",
which can stand for Kalendae, Kalendarum, Kalendis, Kalendas or Kalends.
As a general policy, we should not try to "improve" on the original text,
and leave such an abbreviation as-is.
...
stretching.
Each of these warnings will usually correspond to a visible ugliness in
the output.
...
systems which do no allow different letter cases or spaces in their file names.
See \textcolor{blue}{%
\href{https://en.wikibooks.org/wiki/LaTeX/Basics}%
{LaTeX Basics}%
}, section 'Picking suitable filenames'.
...
The system seems to often prefer "Page" mode, even when "Top" mode is preferred
...
Scaliger will frequently write headers vertically.
...
is generally not the case. To make it longer use the above mentioned
...
\item[p202:12]
  ...
  the sentence appears to continue.
